...
PwsToolbarInfoArray CreatePwsToolbarInfoArray();

const PwsToolbarInfoArray& GetPwsToolbarbuttons()
{
  static PwsToolbarInfoArray tbArray = CreatePwsToolbarInfoArray();
  return tbArray;
}
...
#define SEPARATOR {ID_SEPARATOR, NULL, NULL, NULL, NULL, NULL}
  ...
  _PwsToolbarInfo PwsToolbarButtons[] =
...
#undef PWS_TOOLBAR_BITMAPS
#undef SEPARATOR
  
  PwsToolbarInfoArray tba;
  for (size_t idx = 0; idx < WXSIZEOF(PwsToolbarButtons); ++idx)
    tba.Add(PwsToolbarButtons[idx]);
  
  return tba;
}

#include <wx/arrimpl.cpp>
WX_DEFINE_OBJARRAY(PwsToolbarInfoArray);
